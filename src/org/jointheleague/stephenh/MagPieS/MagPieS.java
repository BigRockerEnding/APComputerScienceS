...
public class MagPieS {
	...
	public String getResponse(String statement) {
		...
		if (statement.isEmpty()) {
			response = "Don't be so quiet";
		} else if (findKeyword(statement, "no", 0) >= 0) {
			response = "Why so negative?";
		} else if (findKeyword(statement, "mother", 0) >= 0 || findKeyword(statement, "father", 0) >= 0 || findKeyword(statement, "sister", 0) >= 0
		...
		} else if (findKeyword(statement, "doctor who", 0) >= 0) {
			response = "OOH! I LIKE DOCTOR WHO!!!";
		} else {
